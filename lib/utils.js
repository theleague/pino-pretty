...
function prettifyMessage ({ log, messageFormat, messageKey = MESSAGE_KEY, colorizer = defaultColorizer, levelLabel = LEVEL_LABEL }) {
  const levelName = log[LEVEL_KEY]

  /* istanbul ignore next */
  const messageColorizer = LEVEL_NAMES[levelName]
    ? colorizer.colored[LEVEL_NAMES[levelName]]
    : colorizer.message
  ...
  if (messageFormat && typeof messageFormat === 'string') {
    ...
    return messageColorizer(message)
  }
  if (messageFormat && typeof messageFormat === 'function') {
    const msg = messageFormat(log, messageKey, levelLabel)
    return messageColorizer(msg)
  }
  if (messageKey in log === false) return undefined
  if (typeof log[messageKey] !== 'string') return undefined
  return messageColorizer(log[messageKey])
}
